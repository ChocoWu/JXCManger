...
public class JXCFrame {
	...
	public JXCFrame() {
		...
		frame.getContentPane().add(navigationPanel, BorderLayout.NORTH);//调整导航标签面板的的位置，在North，south，west，east
	...
	private JTabbedPane createNavigationPanel() { // 创建导航标签面板的方法
		...
		tabbedPane.setFocusable(false);//将此组件的可对焦状态设置为指定的值。此值覆盖组件的默认可聚焦性。
		tabbedPane.setBackground(new Color(211, 230, 192));//导航栏边框的颜色
		tabbedPane.setBorder(new BevelBorder(BevelBorder.RAISED));
		//创建具有指定类型的斜角边框，其颜色将从传递到paintBorder方法的组件的背景颜色派生。

		JPanel baseManagePanel = new JPanel(); // 基础信息管理面板
		baseManagePanel.setBackground(new Color(215, 223, 194));//设置第二个导航栏的背景颜色
		baseManagePanel.setLayout(new BoxLayout(baseManagePanel,
				BoxLayout.X_AXIS));//标签从左到右，Y_AXIS是从上到下
	...
	private JButton createFrameButton(String fName, String cname) {
		...
		/**
		 * 在按钮边框和标签之间设置边距空间。设置为null将使按钮使用默认边距。按钮的默认边框对象将使用此值创建适当的边距。
		 * 但是，如果在按钮上设置了非默认边框，则是Border对象负责创建适当的边距空间（否则此属性将被有效地忽略）。
		 */
		button.setMargin(new Insets(0, 0, 0, 0));
		//设置hideActionText属性，该属性确定按钮是否显示Action中的文本。只有在按钮上安装了操作时，此功能才有用。
		button.setHideActionText(true);
		//设置paintFocus属性，对于要绘制的焦点状态，它必须为true。 paintFocus属性的默认值为true。
		//一些外观和感觉可能不会画焦点状态;他们将忽略此属性。
		button.setFocusPainted(false);
		//设置borderPainted属性。如果为true并且按钮有边框，则边框将被绘制。 borderPainted属性的默认值为true。
		//一些外观和感觉可能不支持borderPainted属性，在这种情况下，他们忽略这一点。
		button.setBorderPainted(false);
		//设置contentAreaFilled属性。如果为true，按钮将绘制内容区域。如果你想有一个透明按钮，例如一个只有图标的按钮，那么你应该设置为false。不要调用setOpaque（false）。 contentAreaFilled属性的默认值为true。
		//此函数可能会导致组件的opaque属性更改。
		//调用此函数的确切行为在逐个组件和L＆F-L＆F基础上不同
		button.setContentAreaFilled(false);
		if (icon_roll != null)
			button.setRolloverIcon(icon_roll);//设置按钮的反转图标
		if (icon_down != null)
			button.setPressedIcon(icon_down);//设置按钮的按下图标
		return button;
	}
	// 获取内部窗体的唯一实例对象
