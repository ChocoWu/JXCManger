...
public class QuanManager extends JInternalFrame {
	...
	public void initComboBox() {
		...
		userCombo.removeAllItems();//从列表中移除所有的项
		for (Iterator iter = khInfo.iterator(); iter.hasNext();) {//迭代器
			List element = (List) iter.next();//强制类型转换，
			Item item = new Item();
	...
	private void doUserSelectAction() {
		...
		if (!(userCombo.getSelectedItem() instanceof Item)) {
			//instanceof是java的关键词，测试它左边的对象是否是它右边的类的实例，返回boolean类型的数据
			return;
		}
