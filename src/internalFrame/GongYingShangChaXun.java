...
public class GongYingShangChaXun extends JInternalFrame {
	...
	public GongYingShangChaXun() {
		...
		dftm.setColumnIdentifiers(tableHeads);//替换模型中的列标识符。
		//newIdentifiers(即tableHeads)的数目大于当前的列数，则会将新列添加到模型中每行的末尾。
		//如果newIdentifiers的数目小于当前列数，则会丢弃行末尾的所有其他列。
		final JScrollPane scrollPane = new JScrollPane(table);
		final GridBagConstraints gridBagConstraints_6 = new GridBagConstraints();
		gridBagConstraints_6.weighty = 1.0;
		gridBagConstraints_6.anchor = GridBagConstraints.NORTH;
		gridBagConstraints_6.insets = new Insets(0, 10, 0, 10);
		//此字段指定组件的外部填充，组件与其显示区域边缘之间的最小空间量。默认值为new Insets（0，0，0，0）。
